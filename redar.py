...
from shooter import shuter_igra 
from labirint import labirint_igrat 
import json 
...
#with open ("baza_data.json", "w", encoding="utf-8") as file: 
#    json.dump(baza,file) 
...
class Ui_MainWindow(object): 
    def setupUi(self, MainWindow): 
        MainWindow.setObjectName("redar") 
        ...
        self.shuter_igra.clicked.connect(shuter_igra) 
        self.labirint_igrat.clicked.connect(labirint_igrat) 
 
         
        self.retranslateUi(MainWindow) 
        QtCore.QMetaObject.connectSlotsByName(MainWindow) 
        self.groupBox_2.hide() 
        self.login.clicked.connect(self.login_baza) 
    def login_baza(self): 
        global baza 
        log = self.lineEdit.text() 
        password = self.lineEdit_2.text() 
        if log in baza and baza[log]==password: 
 
            self.groupBox_2.show() 
            self.groupBox.hide() 
            self.label.setText(str(log)) 
        else: 
 
            baza[log] = password 
 
            with open("baza_data.json", "a") as file: 
                json.dump(baza, file, sort_keys=True, ensure_ascii=False) 
            self.groupBox_2.show() 
            self.groupBox.hide()
...
if name == "__main__": 
    ...
    MainWindow.show() 
    #with open("baza_data.json", "r") as file: 
    #    baza = json.load(file) 
    sys.exit(app.exec_())
